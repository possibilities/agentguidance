...
read-only, and lifecycle actions require authorization covering the exact items and
operations. Existing explicit item or batch authorization counts; the survey
itself does not grant authority to act.
...
Present the evidence and a concrete recommendation for unresolved items.
Ask for the missing scope, preferably one focused item when ownership or
consequences differ. Honor an explicit batch instruction covering named
items or a clear class of worktrees; do not demand separate confirmations
for operations already covered. An approval for one item does not expand to
unrelated items. Recheck each target immediately before acting.
...
When an answer is needed and the human stops responding, leave that decision
pending. Continue only independent work and items already covered by explicit
authorization; silence never broadens the scope.
