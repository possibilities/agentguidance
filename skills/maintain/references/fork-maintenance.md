...
- For a substantial repair, conflict resolution, or cross-cutting change,
  use bounded independent review when the active role permits delegation and
  the risk warrants it. Give reviewers concrete behavior and invariants to
  challenge, and choose model and effort under that role. Repair concrete
  findings or record why they do not apply; report an unavailable review
  when the workshop requires one.
...
Review an offered change before publishing it, using independent reviewers
when the active role permits them or the workshop explicitly requires them.
If a required reviewer is unavailable, prepare the result and leave that gate
pending rather than claiming it passed.

Opening an upstream request and sending its body, comments, or replies require
the human's authorization for that scope. Existing explicit authorization
counts; do not ask for the same decision again. Code changes answering clear
review feedback may proceed within the task, but a recap comment is still an
external message and needs communication authority. A maintenance invocation
does not by itself authorize new upstream correspondence.
